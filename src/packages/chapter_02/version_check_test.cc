...
bool VersionAtLeast(std::string_view actual, std::string_view minimum) {
  ...
  if (a.suffix == m.suffix) return true;
  if (a.suffix.empty() && !m.suffix.empty()) return false;
  if (!a.suffix.empty() && m.suffix.empty()) return true;
...
int CheckVersionedTools() {
  ...
  for (const auto& r : reqs) {
    ...
    if (!VersionAtLeast(*ver, r.minimum_version)) {
      std::cerr << "ERROR: " << r.name << " version " << *ver << " < required " << r.minimum_version << "\n";
      ++failures;
      continue;
    }
